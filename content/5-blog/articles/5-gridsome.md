...
What a world, am I right? Way back in 2018 I was still mildly terrified of Javascript, and today I'm throwing out arrow functions and spreads and destructured variables like candy.
...
Also, webfaction, one of the very few hosts that I've enjoyed using, was [bought out by godaddy](https://community.webfaction.com/questions/21806/webfaction-joins-forces-with-godaddy). That's one of the reasons I switched over to this new stack.
...
Okay, in reality HMR can act up from time to time, but with Gridsome I never noticed it misbehaving.
...
At Datawheel I work in React day to day, and have never particularly liked it — that's its own blog post. I've heard nothing but positive things about Vue, and felt like this was a good opportunity to dive in and give it a spin.
...
There _is_ now an official plugin which allows you to [add Vue components inside your markdown files](https://gridsome.org/plugins/@gridsome/vue-remark), which I found *essential*. I actually started rebuilding this site with Gridsome back in June, but hit a brick wall during which I just had to wait for the plugin to be released. Which brings me to another point:

### Defining routes in...plugin config?

Aside from issues with GraphQL and markdown, my biggest pain point was in configuring the routes. For example, a static page which renders a collection of articles must be defined in the config like:
...
Defining an entire site full of pages and templates individually like this just feels overly verbose and I wish it could be more automatic.
